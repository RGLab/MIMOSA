...
estimateProportions2.MDMixResult <- function(x, method = "mode") {
  ...
  if (method == "mean") {
    ...
    ml <- cbind(pu = prop.table(as.matrix(x$n.unstim), margin = 1)[, 
                                                                   2], ps = prop.table(as.matrix(x$n.stim), margin = 1)[, 
                                                                                                                        2])
...
proportions.icsdata <- function(object) {
  r <- t(apply(object, 1, function(x) cbind(prop.table(x[c("Ns", 
                                                           "ns")])[2], prop.table(x[c("Nu", "nu")])[2])))
  colnames(r) <- c("ps", "pu")
  return(r)
}
...
huberFilter <- function(object, sd = 2) {
  if (any(class(object) == "icsdata")) {
    ...
    difference <- apply(stim, 1, function(x) prop.table(x)[2]) - 
      apply(unstim, 1, function(x) prop.table(x)[2])
    h <- huber(difference)
...
setMethod("pData<-", c("BetaMixResult", "data.frame"), function(object, 
                                                                value) {
...
f0 <- function(p, z, d, w, alternative = alternative, mciter = mciter) {
  -CompleteDataLLRcpp(d = d, alpha0 = p[3] * p[4], beta0 = (1 - 
                                                              p[3]) * p[4], alphaS = p[1] * p[2], betaS = (1 - p[1]) * p[2], 
                      z = z, w = w, alternative = alternative, mciter = mciter)
}
f0m <- function(p, z, d, w, alternative = alternative, mciter = mciter, 
                alpha0, beta0) {
  -CompleteDataLLRcpp(d = d, alpha0 = alpha0, beta0 = beta0, alphaS = p[1] * 
...
f1.log <- function(x, alpha.u, beta.u, alpha.s, beta.s) {
  exp(dbeta(x, alpha.u, beta.u, log = T) + pbeta(x, alpha.s, beta.s, 
                                                 log.p = T, lower.tail = FALSE))
}
f2.log <- function(x, alpha.u, beta.u, alpha.s, beta.s) {
  exp(dbeta(x, alpha.s, beta.s, log = T) + pbeta(x, alpha.u, beta.u, 
                                                 log.p = T, lower.tail = TRUE))
}
...
# thresholds=seq(0,1,by=0.001) d<-r@data; N<-nrow(d)
...
# sensitivity<-calcSens(sensitivity,thresholds,1) #False discovery
# rates
...
# sm.betabin<-0 sm.fisher<-0 for(i in which(truth=='TRUE')){ for(j
# in which(truth!='TRUE')){
# sm.betabin<-sm.betabin+as.numeric(r@z[i,2]>r@z[j,2]) ##less than
# since these are p-values
...
plotPriors <- function(fit, a0 = NULL, b0 = NULL, as = NULL, bs = NULL, 
                       l = 1000, ...) {
  ...
  if (fit@alternative.model == "greater") {
    ...
    null <- dbeta(s, fit@alpha0, fit@beta0) * pbeta(1 - s, fit@betaS, 
                                                    fit@alphaS)
    stim <- dbeta(s, fit@alphaS, fit@betaS) * pbeta(s, fit@alpha0, 
                                                    fit@beta0)
  ...
  if (!skip) {
    ...
    legend("topright", c("estimated null", "estimated stimulated", 
                         "true null", "true stimulated"), lty = c(1, 1, 2, 2), 
...
# ',levels(object@fname),'\n') #Walk the nested list in .Data and
# summarize the levels head<-mode(object@.Data) this<-object@.Data
# levels<-NULL nlevels<-0; while(head=='list'){ nlevels<-nlevels+1
# levels<-c(levels,list(names(this))) this<-this[[1]]
# head<-mode(this) } #Omit reporting the last level corresponding
# to patients cat(nlevels-1,' Levels of nesting\n') for(i in
# 1:(nlevels-1)){ cat(rep(' ',i),i,'. ',levels[[i]],'\n') } })
...
# x<-as(ics,'data.frame') if(!exists('extra.identifiers'))
# extra.identifiers<-NULL
# x<-melt(x,id=c('ID','antigen','parent','fcsfile','fname',extra.identifiers),measure=c('pos','neg'))
# form<- paste(paste('antigen~variable|',
# paste(paste(extra.identifiers,sep = ' ', collapse = '+'),
# ifelse(is.null(extra.identifiers),'', '+'))),'parent + fname +
# ID') x<-cast(x,as.formula(form),fun.aggregate=sum) return(x) })
...
setMethod("show", "BetaMixResult", function(object) {
  ...
  cat("w: Pr(ps=pu)=", object@w[1], ifelse(object@alternative.model == 
                                             "greater", " Pr(ps>pu)=", " Pr(ps!=pu)="), object@w[2], "\n")
})
...
setMethod("summary", "BetaMixResult", function(object, ...) {
  ...
  cat("Positivity: ", table(factor(object@fdr <= threshold, levels = c("FALSE", 
                                                                       "TRUE")))[2], " of ", length(object@fdr), " at ", threshold * 
        100, "% FDR\n")
})
# plot method for BetaMixResult, will plot the log likelihood
# trajectory of the model fitting process
plot.BetaMixResult <- function(x, y, ...) {
  plot(x@traj[-1], main = "Complete Data Log Likelihood Trajectory", 
       xlab = "Iteration", ylab = "Complete Data Log Likelihood", 
       type = "o")
}
