...
setMethod("estimateProportions", signature = c("BetaMixResult"), function(bmr, 
                                                                          method = "posterior", N = 10000, burn = 1000, volcano = FALSE) {
  method <- match.arg(method, c("posterior", "ML"))
  if (method %in% "posterior") {
    r <- t(sapply(1:nrow(bmr@data), function(i) {
      colMeans(gibbsPsPu(bmr@data, inits = list(w = bmr@w, z = bmr@z, 
                                                alpha0 = bmr@alpha0, beta0 = bmr@beta0, alphaS = bmr@alphaS, 
                                                betaS = bmr@betaS), alt = bmr@alternative.model, which = i, 
                         N = N, burn = burn, z = NULL, volcano = volcano))
    }))
  } else {
    r <- data.frame(Pu = bmr@data[, "nu"]/(bmr@data[, "nu"] + 
                                             bmr@data[, "Nu"]), Ps = bmr@data[, "ns"]/(bmr@data[, "ns"] + 
                                                                                         bmr@data[, "Ns"]))
  }
  return(r)
})

gibbsPsPu <- function(curdat = NULL, inits = NULL, which = NULL, N = 10000, 
                      burn = 5000, alt, z = NULL, ecdf.approx = FALSE, volcano = FALSE, 
                      ...) {
  ...
  for (i in 1:N) {
    ...
    # TODO estimates of ps and pu given model 2 should be different
    # when alternative model changes
    if (alt == "greater") {
      ps[i] <- qbeta(runif(1, pbeta(pu[i], alphaS + ns, betaS + 
                                      Ns, log.p = F), 1), alphaS + ns, betaS + Ns, log.p = F)
    } else {
      ps[i] <- rbeta(1, alphaS + ns, betaS + Ns)
    }
  }
  # Sometimes qbeta and pbeta don't give a decent answer when very
  # near the tails. Approximate using the ecdf. We'll say we need at
  # least 2000 samples
  if (ecdf.approx) {
    if (N < 2000) {
    ...
    e <- ecdf(ps)
    for (i in 1:N) {
      ps[i] <- qbeta(runif(1, e(pu[i]), 1), alphaS + ns, betaS + 
                       Ns)
    }
    
  }
  # pu<-inits$w[1]*PU+inits$w[2]*pu ps<-PS*inits$w[1]+ps*inits$w[2]
  # Given the data, compute which is the most likely generating
  # cluster, and return simulations from that. If z is not given
  if (volcano) {
    return(cbind(pu = inits$z[which, 1] * PU + inits$z[which, 
                                                       2] * pu, ps = inits$z[which, 1] * PS + inits$z[which, 
                                                                                                      2] * ps))
  }
  if (is.null(z)) {
    if (alt == "greater") {
      m1 <- exp(.Call("MarginalNULL", ns = ns, Ns = Ns, nu = nu, 
                      Nu = Nu, alpha0 = rep(inits$alpha0, length(ns)), beta0 = rep(inits$beta0, 
                                                                                   length(ns)), alphaS = rep(inits$alphaS, length(ns)), 
                      betaS = rep(inits$betaS, length(ns)), inits$w, log = TRUE, 
                      package = "MIMOSA"))
      m2 <- exp(.Call("MarginalGT", ns = ns, Ns = Ns, nu = nu, 
                      Nu = Nu, alpha0 = rep(inits$alpha0, length(ns)), beta0 = rep(inits$beta0, 
                                                                                   length(ns)), alphaS = rep(inits$alphaS, length(ns)), 
                      betaS = rep(inits$betaS, length(ns)), inits$w, log = TRUE, 
      ...
      m1 <- exp(.Call("MarginalNULL", ns = ns, Ns = Ns, nu = nu, 
                      ...
                      betaS = rep(inits$betaS, length(ns)), inits$w, log = TRUE, 
                      package = "MIMOSA"))
      m2 <- exp(.Call("MarginalNE", ns = ns, Ns = Ns, nu = nu, 
                      Nu = Nu, alpha0 = rep(inits$alpha0, length(ns)), beta0 = rep(inits$beta0, 
                                                                                   length(ns)), alphaS = rep(inits$alphaS, length(ns)), 
                      betaS = rep(inits$betaS, length(ns)), inits$w, log = TRUE, 
                      package = "MIMOSA"))
    ...
    # if z is given, mix the PS,PU nd ps, pu samples in the
...
## #ps[i]<-inits$w[2]*qbeta(runif(1,pbeta(pu[i],alphaS+ns-1,betaS+Ns-1),1),alphaS+ns-1,betaS+Ns-1)+inits$w[1]*pu[i]
## #TODO estimates of ps and pu given model 2 should be different
## when alternative model changes if(alt=='greater'){
...
## #ps<-PS*inits$w[1]+ps*inits$w[2] #Given the data, compute which
## is the most likely generating cluster, and return simulations
## from that.  if(alt=='greater'){
...
## cbind(pu=PU,ps=PS)[(burn+1):N,] }else{
## cbind(pu=pu,ps=ps)[(burn+1):N,] } }

# Given z, returns the q-values for each observation. The q-value
# for observation j is the false discovery rate for observations
# with q<q_j
