...
##		 cm<-colMeans(p)
##		 lims<-rbind(cm-apply(p,2,sd)*5,cm+apply(p,2,sd)*5)
##		 k<-k;
...
#		 xup<-d@data[k,"ns"]*2
#		 yup<-d@data[k,"nu"]*2
#		 #If counts are really small, go from zero to 10.
#		 if(xup<10){
#			 xup<-10
#		 }
#		 if(yup<10){
...
#		 
##		 #this should be tot adjusted by alpha0 and beta0 depending on the generating model, which depends on z
##		 if(d@z[k,1]>0.5){
##			 #ps=pu model
...
##			y1.i<-unique(round(y1*tot.y1-d@alpha0))
##			#unique indices
##			#sort out which model this came from and compute ns and nu
...
##			tot.y1<-d@data[k,"nu"]+d@data[k,"Nu"]+d@alpha0+d@beta0
##			#integer counts of ns and nu
##			x1.i<-unique(round(x1*tot.x1-d@alphaS))
##			y1.i<-unique(round(y1*tot.y1-d@alpha0))
##			#unique indices
##			#sort out which model this came from and compute ns and nu
...
##		 y1<-y1/(d@data[k,"nu"]+d@data[k,"Nu"])
#		 
#		 #comment out R based code
...
#	M<-sqrt(exp(m1)+exp(m2))
#	M<-matrix(M,nrow=length(x1))
#		 #par(mfrow=c(1,2))
