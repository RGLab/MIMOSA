...
initBetaMix <-
	...
	if(fixedNULL&!is.null(ics)){
		...
		pars<-try(optim(par=(c(alpha0,beta0)),function(p){-sum(lchoose(d[,"pos"]+d[,"neg"],d[,"pos"])-lbeta((p[1]),(p[2]))+lbeta(d[,"pos"]+(p[1]),d[,"neg"]+(p[2])))},method="L-BFGS-B",lower=c(0.1,0.1),upper=c(Inf,Inf)))
		if(inherits(pars,"try-error")|try(pars$convergence,silent=TRUE)!=0){
		...
		pars<-try(optim(par = c(alpha0, beta0), function(p) {-sum(lchoose(d[, "nu"] + d[, "Nu"], d[, "nu"]) - lbeta(p[1],p[2]) + lbeta(d[, "nu"] + p[1], d[, "Nu"] + p[2]))}, method = "L-BFGS-B", lower = c(0.1, 0.1), upper = c(Inf,Inf)))
		if(inherits(pars,"try-error")|try(pars$convergence,silent=TRUE)!=0){
			V<-var(d[,"nu"]/(d[,"nu"]+d[,"Nu"]))
			mu0<-mean(d[,"nu"]/(d[,"nu"]+d[,"Nu"]))
			alpha0<-mu0*(mu0*(1-mu0)/V - 1)
	...
	if(!fixedNULL){
		...
		pars<-try(optim(par=(c(alpha0,beta0)),function(p){-sum(lchoose(d[,"nu"]+d[,"Nu"],d[,"nu"])-lbeta((p[1]),(p[2]))+lbeta(d[,"nu"]+(p[1]),d[,"Nu"]+(p[2])))},method="L-BFGS-B",lower=c(0.1,0.1),upper=c(Inf,Inf)),silent=TRUE)
		if(inherits(pars,"try-error")|try(pars$convergence,silent=TRUE)!=0){
			pars<-try(optim(par=log(c(alpha0,beta0)),function(p){
								-sum(lchoose(d[,"nu"]+d[,"Nu"],d[,"nu"])-lbeta(p[1],p[2])+lbeta(d[,"nu"]+exp(p[1]),d[,"Nu"]+exp(p[2])))
	...
	#filter out anything that has ns and nu == 0 
	#now estimate responding, stimulated samples
	...
	if(l>1){
		...
		w<-c(1-(sum(fisher.p.w)+priorXi-1)/(length(fisher.p)+priorXi-1),(sum(fisher.p.w)+priorXi-1)/(length(fisher.p)+priorXi-1))
		...
		if(alternative=="greater"){
			...
			#f0 is the mean,sample size parameterization, both components
			#f0m is for the stimulated component only
		...
		##non-informative?
		#TODO also figure out the right initialization here.
		alphaS<-1.01
		betaS<-1.01
		muS<-mu0
		#initialize z and w
	...
	return(list(z=z,w=w,alpha0=alpha0,beta0=beta0,alphaS=alphaS,betaS=betaS,muS=muS,mu0=mu0))
}
