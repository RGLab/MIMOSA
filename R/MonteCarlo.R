...
simAlpha.s <- function(alpha.s, alpha.u, z, S, llnull, llresp, i, 
                       rate) {
  a <- llnull(c(alpha.s, alpha.u))
  b <- llresp(c(alpha.s, alpha.u))
  
  old <- sum(a * z[, 1] + b * z[, 2]) + dexp(alpha.s[i], 1e-04, 
                                             log = TRUE)
  ...
  new <- sum(a * z[, 1] + b * z[, 2]) + dexp(alpha.s.prop[i], 1e-04, 
                                             log = TRUE)
...
simAlpha.u <- function(alpha.s, alpha.u, z, S, llnull, llresp, i, 
                       rate) {
  a <- llnull(c(alpha.s, alpha.u))
  b <- llresp(c(alpha.s, alpha.u))
  
  old <- sum(a * z[, 1] + b * z[, 2]) + dexp(alpha.u[i], 1e-04, 
                                             log = TRUE)
  ...
  new <- sum(a * z[, 1] + b * z[, 2]) + dexp(alpha.u.prop[i], 1e-04, 
                                             log = TRUE)
...
simZ <- function(q, alpha.s, alpha.u, llnull, llresp) {
  ...
  den <- apply(cbind(a, b), 1, function(x) log(sum(exp(x - max(x)))) + 
                 max(x))
  p <- exp(a - den)
  z <- sapply(p, function(p) sample(c(0, 1), 1, prob = c(1 - p, 
                                                         p), replace = FALSE))
  return(cbind(z, 1 - z))
}
...
icsdata2mvicsdata <- function(x) {
  if (any(class(x) %in% "icsdata")) {
    xx <- list(n.stim = x[, c("Ns", "ns")], n.unstim = x[, c("Nu", 
                                                             "nu")])
...
#'  @name .fitMCMC
#'  @export
.fitMCMC <- function(data, inits = NULL, iter = 250000, burn = 50000, 
                     thin = 1, tune = 100, outfile = basename(tempfile(tmpdir = ".", 
                                                                       fileext = ".dat")), alternative = "greater", UPPER = 0.5, 
                     LOWER = 0.15, FAST = TRUE, EXPRATE = 1e-04, pXi = 1, seed = 10) {
  set.seed(seed)
  ...
  # If the alternative hypothesis is one-sided, then compute a
  # filter for pu>ps and pass that to the MCMC code
  ...
  result <- .Call("fitMCMC", as.matrix(data$n.stim), as.matrix(data$n.unstim), 
                  ...
                  as.matrix(inits$z), as.vector(iter), as.vector(burn), as.vector(thin), 
                  as.numeric(tune), as.character(outfile), as.vector(filter), 
                  as.numeric(UPPER), as.numeric(LOWER), FILTER, FAST, as.numeric(EXPRATE), 
  ...
  # which.cols<-nc/3+1:nc #awk each co lumn and read it
  ...
  result$getP <- function(x = paste(outfile, "P", sep = ""), thin = 1) {
    if (thin > 1) {
      ...
      thins <- paste("p", paste(rep(";n", thin - 1), collapse = ""), 
                     sep = "")
      s <- sprintf("sed -n '%s' %s|cut -f %s-%s", thins, x, 
                   (nc/3 + 1), nc)
      con <- pipe(s)
      d <- do.call(rbind, lapply(strsplit(readLines(con), "\t")[-1L], 
                                 as.numeric))
      ...
      nc <- ncol(d)
      d <- split(as.list(data.frame(d[, (nc/3 + 1):nc])), gl(nc/3, 
                                                             2))
    }
    d
  }
