...
constrainedNRLL <- function(data.stim, data.unstim, z = NULL) {
  ...
  function(pars) {
    # TODO make general for multiple components (i.e. more than two
    # columns in z).
    s <- pars[1]
    lambda <- pars[2]
    w <- pars[-c(1:2)]
...
# Gradient for constrained log likelihood for the non responder
# component, parameterized as n*s
...
gradCostrainedNRLL <- function(data.stim, data.unstim, z = NULL) {
  ...
  function(pars) {
    # TODO make general for multiple components (i.e. more than two
    # columns in z). calculate things once
    n <- pars[1]
    lambda <- pars[2]
    s <- pars[-c(1:2)]
    nc <- ncol(data)
    ...
    gn <- colSums(s * digamma(ns) - s * digamma(data)) + ss * 
      digamma(csd) - ss * digamma(sns)
...
# Hessian of the constrained log likelihood for the non responder
# component
hessConstrainedNRLL <- function(data.stim, data.unstim, z = NULL) {
  ...
  function(pars) {
    # TODO make general for multiple components (i.e. more than two
    # columns in z). Calculate everything once
    n <- pars[1]
    lambda <- pars[2]
    s <- pars[-c(1:2)]
    ns <- n * s
    ...
    d2n <- (colSums(s2 * tns - s2 * tdata) + ss^2 * tcsd - ss^2 * 
              tsns) %*% z[, 1]
    d2l <- 0
    d2s <- n2 * ((tns - t(t(td) - tcsd + tsns)) %*% z[, 1])
    D <- diag(c(d2n, d2l, d2s))
    dndl <- 0
    dnds <- t(t(dns + ns * tns - ddata - ns * tdata) + dcsd + 
                n * ss * tcsd - dsns - n * ss * tsns) %*% z[, 1]
    dsds <- (n2 * tcsd - n2 * tsns) %*% z[, 1]
    dsdl <- rep(1, ncol(data)) %*% z[, 1]
    lt <- c(dndl, dnds, rep(dsdl, length(s)), rep(dsds, sum(1:(length(s) - 
                                                                 1))))
...
constrainedRLL <- function(data.stim, data.unstim, z = NULL) {
  ...
  function(pars) {
    ...
    ll <- (colSums(gsws + gswu - gdats - gdatu) + gcsdats + gcsdatu - 
             gssws - gsswu + lambda1 * (sum(wu) - 1) + lambda2 * (sum(ws) - 
                                                                    1)) %*% z[, 2]
    return(ll)
  }
}

# gradient for the log likelihood of the constrained responder
# component
gradConstrainedRLL <- function(data.stim, data.unstim, z = NULL) {
  
}

# hessian for the log likelihood of the constrained responder
# component
...
test <- function(x) {
  ...
  time2 <- system.time({
    ...
    repeat {
      ...
      if (norm(new - guess) < 0.01) {
        ...
        names(guess.closedform) <- c("N", "lambda", "w1", 
                                     "w2", "w3", "w4")
        break
      } else guess <- new
    }
  ...
  time1 <- system.time({
    ...
    repeat {
      new <- guess - solve(hessian(ll, guess)) %*% grad(ll, 
                                                        guess)
      if (norm(new - guess) < 0.01) {
        guess.numerical <- guess
        names(guess.numerical) <- c("N", "lambda", "w1", "w2", 
                                    "w3", "w4")
        break
  ...
  cat("Estimates from closed form derivatives:\n\t ", guess.closedform[-2L], 
      "\n")
  optres <- try(optim(fn = ll, par = c(mean(rowSums(dat[[1]])), 
                                       1, prop.table(colSums(dat[[1]]))), method = "L-BFGS-B", gr = gr, 
                      lower = c(0, -Inf, 1e-10, 1e-10, 1e-10, 1e-10), upper = c(Inf, 
                                                                                Inf, 1, 1, 1, 1)), silent = TRUE)
