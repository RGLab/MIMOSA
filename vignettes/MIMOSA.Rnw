...
<<extract>>=
data<-flowContrasts(HVTN049.ics, control="negctrl1",stim="ENV-1-PTEG",subset=c("cd4","IFNg"))
head(data)
@
...
Printing the fitted displays the contrast of interest, the number of samples in the model, the log--likelihood of the final model, and the estimates of the component weights. In this case, the ENV stimulated samples are almost all responders (97\%). 

We can extrace q--values (the false discovery rates) for each observation in the model via the \textit{fdr} object slot. At the 1\% level we obtain:
<<fdr>>=
table(result@fdr<0.01)
@

We see \Sexpr{table(result@fdr<0.01)[2]} of 27 individuals are called responders at the 1\% level, and we would expect 1\%  of them to be false positives. Another way to visualize the data is via a volcano plot, which plots the false discovery rate on the y--axis vs the difference between the stimulated and unstimulated samples on the x--axis. 
...
ifng<-BetaMix(flowContrasts(HVTN049.ics,stim="ENV-1-PTEG",control="negctrl1",subset=c("cd4","IFNg")),scl=9)
il2<-BetaMix(flowContrasts(HVTN049.ics,stim="ENV-1-PTEG",control="negctrl1",subset=c("cd4","IL2")),scl=9)
@
...
Comparing the polyfunctional fit vs the monofunctional responder calls, we see, as expected, fewer subjects exhibiting polyfunctional cytokine expression than monofunctional cytokine expression.
