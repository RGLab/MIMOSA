...
<<sims,eval=TRUE,echo=FALSE,fig.keep='high'>>=
require(MIMOSA)
set.seed(100)
...
ggplot(ICS)+geom_histogram(aes(x=CYTNUM,fill=(ANTIGEN%in%"negctrl")),alpha=0.5,position="identity")+facet_wrap(~Ntot)
@
