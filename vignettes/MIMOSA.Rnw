...
dat<-melt(within(ICS,antigen[antigen%in%c("negctrl1","negctrl2")]<-"negctrl1"))
...
E<-ExpressionSet(assayData=DAT,featureData=AnnotatedDataFrame(data=data.frame(features)),phenoData=AnnotatedDataFrame(data=pheno))
@

The lions' share of the work is just formatting the data. Fitting the model is then trivial. Here, our treatment group is "STIM", and our control group is "CONTROL". We fit our model separately for each T cell subset. 
